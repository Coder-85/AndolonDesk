...
public class AttendingProtestController {
    ...
    private List<ProtestItem> loadAttendingProtests(int id) throws SQLException {
        ...
        String sql = "SELECT p.*, u.name AS author_name, (SELECT COUNT(*) FROM attending_protests ub2 WHERE ub2.protest_id = p.id) AS attending_count FROM protests p INNER JOIN attending_protests b ON p.id = b.protest_id INNER JOIN users u ON u.id = p.author_id WHERE b.user_id = ? ORDER BY p.posted_date DESC";

        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, id);
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ProtestItem item = new ProtestItem(rs.getString("author_name"), rs.getInt("author_id"), rs.getString("posted_date"), rs.getString("title"), rs.getString("event_date"), rs.getString("summary"), rs.getString("description"), rs.getString("category"), rs.getInt("member_count"), rs.getInt("attending_count"), rs.getString("img_name"));
