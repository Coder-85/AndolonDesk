...
public class AttendingProtestController {
    ...
    private List<ProtestItem> loadAttendingProtests(int id) throws SQLException {
        ...
        String sql = "SELECT p.*, u.name AS author_name, (SELECT COUNT(*) FROM attending_protests ub2 WHERE ub2.protest_id = p.id) AS attending_count FROM protests p INNER JOIN attending_protests b ON p.id = b.protest_id INNER JOIN users u ON u.id = p.author_id WHERE b.user_id = ? ORDER BY p.created_at DESC";
